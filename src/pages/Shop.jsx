...
export default function Shop() {
  const { selectedCategory, setSelectedCategory, pets, activePetId } = useAppStore()
  ...
  return (
    <PageWrapper>
      ...
      <div className="bg-gradient-to-b from-primary-deep to-primary pt-12 pb-5 px-4">
        ...
        {/* Search bar */}
        <div className="bg-white/15 backdrop-blur-sm rounded-xl flex items-center gap-2 px-4 py-3">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2">
            <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
          </svg>
          <span className="text-white/60 text-sm">상품 검색...</span>
        </div>
      </div>
      ...
      <div className="px-4 py-4 space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <div>
              <h3 className="font-bold text-gray-800">{pet.name}를 위한 추천</h3>
              <p className="text-xs text-gray-500">{pet.allergy} 알러지 고려 · {pet.breed} 맞춤</p>
            </div>
            ...
            {categories.map(cat => (
              <motion.button
                ...
                className={`flex-shrink-0 px-4 py-2 rounded-full text-sm font-semibold transition-all
                  ${selectedCategory === cat ? 'bg-primary text-white shadow-md shadow-blue-200' : 'bg-white text-gray-500 border border-gray-200'}`}
                whileTap={{ scale: 0.94 }}
              >
                {cat}
              </motion.button>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm text-gray-600 font-medium">총 {filtered.length}개</p>
